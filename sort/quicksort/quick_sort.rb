def quick_sort(list, start_point = nil, end_point = nil)
  ...
  while left < right # The magic starts here
    if pivot == left # Start form the left
      if list[pivot] <= list[right] # If the pivot is less than or equal to the right variable, move it left by one
        right -= 1
      ...
      if list[pivot] >= list[left] # If the pivot is greater than or equal to the left variable, move right by one
        left += 1
